...
class AStar(object):
    ...
    def is_free(self, x):
        ...
        # occu = DetOccupancyGrid2D(self.occupancy)
        x = np.asarray(x)
        flag = self.occupancy.is_free(x)
        return flag
    ...
    def distance(self, x1, x2):
        ...
        x1_p = np.array(x1)
        x2_p = np.array(x2)
        dist = np.sqrt((x1_p - x2_p) @ (x1_p - x2_p))
        return dist
    ...
    def get_neighbors(self, x):
        ...
        sq_reso = reso / math.sqrt(2) # ????????????? may have problem ??????????????????
        four_dirc = [[1, 0], [-1, 0], [0, 1], [0, -1]]
        four_diag = [[1, 1], [1, -1], [-1, 1], [-1, -1]]
        for i in range(4):
            dirc_now = self.snap_to_grid([x[0] + reso*four_dirc[i][0], x[1] + reso*four_dirc[i][1]])
            if self.is_free(dirc_now) ==True:
                neighbors.append(dirc_now)
                
            diag_now = self.snap_to_grid([x[0] + sq_reso*four_diag[i][0], x[1] + reso*four_diag[i][1]])
            if self.is_free(diag_now) ==True:
                neighbors.append(diag_now)
